...
func _run() -> void:
	...
	_expect(encoded.length() == RenderedDashboardAsset.EXPECTED_BASE64_LENGTH, "Rendered dashboard payload normalizes to the exact verified length")
	...
	var controller_script: Script = load("res://scripts/rendered_dashboard_controller_live.gd") as Script
	_expect(controller_script != null, "Rendered dashboard live interaction controller loads")
	...
	if scene != null:
		...
		_expect(controller != null and controller.get_script() == controller_script, "Station uses the rendered dashboard live controller")
		if controller != null:
			...
			if missions_button != null:
				...
				await process_frame
				_expect(rendered_layer != null and rendered_layer.layer == 15, "Returning restores the rendered station canvas layer")
				_expect(return_button == null or not return_button.visible, "Returning hides the station return control")
		instance.queue_free()
	await process_frame
	if failures == 0:
