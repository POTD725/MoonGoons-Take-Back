...
func _draw() -> void:
	_draw_environment_skins()
	_draw_patrol_skins()
	_draw_enemy_skin()

func _draw_environment_skins() -> void:
	# Existing lunar props from Crime Wars give the arena its established visual language.
...
func _draw_patrol_skins() -> void:
	var parent_node: Node = get_parent()
	if parent_node == null:
		return
	var units_value: Variant = parent_node.get("officer_units")
	...
	for unit_value: Variant in units:
		...
		var art_rect: Rect2 = Rect2(center + Vector2(-48.0, -52.0), Vector2(96.0, 96.0))
		if bool(unit.get("covering", false)) and hp > 0:
			draw_circle(center, 52.0 + sin(_pulse * 4.0) * 2.0, Color(0.36, 0.92, 1.0, 0.16))
			draw_arc(center, 51.0, -2.6, 0.55, 30, Color(0.65, 0.97, 1.0, 0.84), 4.0)
		_draw_skin(skin_name, art_rect, tint)

func _draw_enemy_skin() -> void:
	var parent_node: Node = get_parent()
	if parent_node == null:
		return
	var enemy_hp: int = int(parent_node.get("enemy_hp"))
	var center: Vector2 = Vector2(717.0, 322.0)
	var alpha: float = 1.0 if enemy_hp > 0 else 0.30
	var enemy_tint: Color = Color(1.0, 0.22, 0.43, alpha)
	var aura: float = 64.0 + sin(_pulse * 3.2) * 4.0
	draw_circle(center, aura, Color(1.0, 0.12, 0.32, 0.12 if enemy_hp > 0 else 0.03))
	_draw_skin("pulse_cannon", Rect2(center + Vector2(-70.0, -66.0), Vector2(140.0, 140.0)), Color(1.0, 0.22, 0.38, 0.48 * alpha))
	_draw_skin("patrol_deputy", Rect2(center + Vector2(-55.0, -58.0), Vector2(110.0, 110.0)), enemy_tint)
