extends Node3D
## Living MoonGoons precinct city. The management systems remain data-driven,
## while the player interacts with a full 3D station and animated personnel.
...
var camera_target:Vector3 = Vector3(0.0,0.0,0.0)
...
func _build_moon_surface() -> void:
	...
	for crater_index in range(18):
		var crater := _cylinder_mesh(1.1+float(crater_index%4)*0.55,0.08,Vector3(-28.0+float((crater_index*7)%56),-0.1,-18.0+float((crater_index*11)%36)),"#101824",0.0)
		world_root.add_child(crater)
	for star_index in range(70):
		var beacon := _sphere_mesh(0.035+float(star_index%3)*0.018,Vector3(-36.0+float((star_index*13)%72),12.0+float((star_index*17)%26),-30.0-float(star_index%5)),"#BEEBFF",0.6)
		world_root.add_child(beacon)

func _build_corridors() -> void:
	world_root.add_child(_box_mesh(Vector3(39.5,0.22,4.2),Vector3(0.0,0.0,0.0),"#203950",0.02))
...
func _build_exterior() -> void:
	var pad := _cylinder_mesh(6.2,0.35,Vector3(25.2,0.0,7.2),"#243D51",0.0)
	world_root.add_child(pad)
	for ring_radius in [5.5,4.0,2.3]:
		var ring := _torus_mesh(float(ring_radius),0.10,Vector3(25.2,0.22,7.2),"#35D8FF",0.65)
	...
	var tower := _cylinder_mesh(1.1,7.0,Vector3(-25.0,3.4,-10.0),"#23374D",0.0)
	world_root.add_child(tower)
	world_root.add_child(_sphere_mesh(1.4,Vector3(-25.0,7.25,-10.0),"#55DFFF",0.35))

func _rebuild_rooms() -> void:
	for child:Node in rooms_root.get_children():
		child.queue_free()
	room_nodes.clear()
	for index in range(ROOM_IDS.size()):
		var room_id:String = ROOM_IDS[index]
		var room_data:Dictionary = PrecinctState.get_room(room_id)
		var room_node:Node3D = PrecinctRoomFactory.build_room(room_id,room_data)
		room_node.position = ROOM_POSITIONS[room_id] as Vector3
		if index >= 4:
			room_node.rotation.y = PI
...
func _spawn_agents() -> void:
	...
	for officer:Dictionary in PrecinctState.officers:
		...
		roster.append(data)
	for extra:Dictionary in EXTRA_WORKERS:
		roster.append(extra.duplicate(true))
	for index in range(roster.size()):
		var start := Vector3(18.5+float(index%3)*0.8,0.0,3.2+float(index/3)*0.8)
		var agent := PrecinctAgent.new()
		agent.name = "Agent_%d" % index
		agent.configure(roster[index],start)
...
func _initial_room_for(data:Dictionary,index:int) -> String:
	...
	if not assigned.is_empty() and room_nodes.has(assigned):
		return assigned
...
func _route_idle_agents() -> void:
	for index in range(agents.size()):
		...
		if not bool(PrecinctState.get_room(destination).get("repaired",false)):
			destination = "ops"
...
func _route_agent(agent:PrecinctAgent,room_id:String) -> void:
	...
	for child:Node in room_node.get_children():
		if child is Marker3D and child.name.begins_with("Job"):
			job_markers.append(child as Marker3D)
	if door == null:
		return
	var job:Marker3D = door
	if not job_markers.is_empty():
		job = job_markers[absi(agent.officer_id.hash())%job_markers.size()]
	var route:Array[Vector3] = []
	route.append(Vector3(agent.position.x,0.0,0.0))
	route.append(Vector3(door.global_position.x,0.0,0.0))
	route.append(Vector3(door.global_position.x,0.0,door.global_position.z))
	route.append(Vector3(job.global_position.x,0.0,job.global_position.z))
...
func _pick_world(screen_position:Vector2) -> void:
	...
	if collider == null or not collider.has_meta("room_id"):
		return
...
func _build_interface() -> void:
	...
	command_button.text = "COMMAND BOARD"
	command_button.custom_minimum_size = Vector2(164.0,36.0)
	command_button.pressed.connect(_open_command_board)
	nav.add_child(command_button)
...
func _refresh_interface() -> void:
	...
	if not assigned_id.is_empty():
		assigned_name = str(PrecinctState.get_officer(assigned_id).get("name",assigned_id))
...
func _repair_selected() -> void:
	var result:Dictionary = PrecinctState.repair_room(selected_room_id)
	_action_result(result,"repair")

func _upgrade_selected() -> void:
	var result:Dictionary = PrecinctMeta.upgrade_room(selected_room_id)
	_action_result(result,"upgrade")
...
func _assign_selected() -> void:
	...
	if bool(result.get("ok",false)):
		for agent:PrecinctAgent in agents:
			if agent.officer_id==str(officer.get("id","")):
				_route_agent(agent,selected_room_id)

func _train_officer() -> void:
	if PrecinctState.officers.is_empty(): return
	var result:Dictionary = PrecinctMeta.train_officer(str(PrecinctState.officers[selected_officer_index].get("id","")))
	_action_result(result,"upgrade")

func _heal_officer() -> void:
	if PrecinctState.officers.is_empty(): return
	var result:Dictionary = PrecinctMeta.heal_officer(str(PrecinctState.officers[selected_officer_index].get("id","")))
	_action_result(result,"confirm")
...
func _dispatch_patrol() -> void:
	if PrecinctState.patrol_calls.is_empty():
		...
		return
	var selected_indices:PackedInt32Array = team_list.get_selected_items()
	var ids:Array[String] = []
	for item_index:int in selected_indices:
		if item_index>=0 and item_index<PrecinctState.officers.size():
			ids.append(str(PrecinctState.officers[item_index].get("id","")))
